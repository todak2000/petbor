...
def signup(request):
    try:
        ...
        reg_field = [firstName,lastName,phoneNumber,email,password]
        if not None in reg_field and not "" in reg_field:
            if User.objects.filter(phone =phoneNumber).exists() or User.objects.filter(email =email).exists():
                ...
                #generate user_id
                userRandomId = string_generator.alphanumeric(5)
                #encrypt password
                ...
                mail_subject = 'Activate your Petbor account.'
                email = {
                    'subject': mail_subject,
                    'html': '<h4>Hello, '+firstName+'!</h4><p>Kindly use the Verification Code below to activate your Petbor Account</p> <h1>'+code+'</h1>',
                    'text': 'Hello, '+firstName+'!\nKindly use the Verification Code below to activate your Petbor Account',
                    'from': {'name': 'Petbor App', 'email': 'donotreply@wastecoin.co'},
                    'to': [
        ...
        if not None in field and not "" in field:
            if User.objects.filter(user_id =user_id).exists():
                ...
                if code:
                    ...
                    resentEmail = {
                        'subject': mail_subject,
                        'html': '<h4>Hello, '+firstName+'!</h4><p>Kindly find the Verification Code below sent again to activate your Petbor Account</p> <h1>'+code+'</h1>',
                        'text': 'Hello, '+firstName+'!\nKindly find the Verification Code below sent againto activate your Petbor Account',
                        'from': {'name': 'Petbor App', 'email': 'donotreply@wastecoin.co'},
                        'to': [
    ...
    try:
        ...
        if not None in field and not "" in field:
            if User.objects.filter(email =email).exists():
                ...
                if getOtp:
                    # Resend mail using SMTP
                    mail_subject = 'Reset your Petbor account Password Confirmation.'
                    resentEmail = {
                        'subject': mail_subject,
                        'html': '<h4>Hi, '+firstName+'!</h4><p>Kindly find the Reset Code below to confirm that intend to change your Petbor Account Password</p> <h1>'+getOtp.password_reset_code+'</h1>',
                        'text': 'Hello, '+firstName+'!\nKindly find the Reset Code below to confirm that intend to change your Petbor Account Password',
                        'from': {'name': 'Petbor App', 'email': 'donotreply@wastecoin.co'},
                        'to': [
                            {'name': firstName, 'email': email}
                        ]
...
@api_view(["GET"])
# @autentication.token_required_user_id
